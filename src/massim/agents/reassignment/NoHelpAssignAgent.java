 ...
 *
 */
public class NoHelpAssignAgent extends Agent {
	 ...
	 */
	public NoHelpAssignAgent(int id, CommMedium comMed) {
	...
	protected void initializeRound(Board board, int[][] actionCostsMatrix) {
		...
		if(reassigning)
			state = AgentStates.S_RA_INIT;
		else 
			state = AgentStates.S_INIT;
	...
	protected AgCommStatCode sendCycle() {
		...
		if (mySubtask() == -1)
		{
			logInf("In Send cylce: No Subtask to Do! Sleeping!");
			return returnCode;
		}
		...
		case S_INIT:
			if (!reachedGoal())
			{
				...
				if (cost  <= resourcePoints())
					setState(AgentStates.R_MOVE);
				else
					setState(AgentStates.R_BLOCKED);				
			}
			else
				setState(AgentStates.R_SKIP);
	...
	protected AgCommStatCode receiveCycle() {
		...
		if (mySubtask() == -1)
		{
			logInf("In Recieve cycle: No Subtask to Do! Sleeping!");
	...
	protected AgGameStatCode finalizeRound() {
		...
		if (mySubtask() == -1)
		{
			logInf("No Subtask to Do! Subtask ====== -1");
		...
		if (reachedGoal())
		{
			logInf("Reached the goal! Hoooooray");
	...
	private void reSendCycle() {
		...
		case S_RA_INIT:
			...
			if (mySubtask() != -1)
			{
				...
				needReassign = (twb <= NoHelpAssignAgent.WREASSIGN ) ;//&&
				//wellbeing <= NoHelpAssignAgent.WREASSIGNREQ;
			}
			...
			*/
			setState(AgentStates.R_RA_GET_REQ);
			break;
		case S_RA_CMD:
			/*
		...
		case S_RA_REPORT_ESTIMATE:
			...
			setState(AgentStates.R_RA_GATHER);
			break;
		case S_RA_CONT:
			setState(AgentStates.R_RA_CONT);
			break;
		case S_RA_ASSIGN:
			if (leaderAgent == id())
			{
			...
			setState(AgentStates.R_RA_ASSIGN);
			break;
		default:
	...
	private boolean reReceiveCycle() {
		...
		case R_RA_GET_REQ:
			...
			*/
			setState(AgentStates.S_RA_CMD);
			break;
		case R_RA_CONT:
			setState(AgentStates.S_INIT);
			break;
		case R_RA_CMD:
			/*
			msgStr = commMedium().receive(id());
			...
			*/
			if (reassigning)
			{
				...
				setState(AgentStates.S_RA_REPORT_ESTIMATE);	
				//added
				reassigning = false;
			}
			else
				setState(AgentStates.S_RA_CONT);			
			break;
		case R_RA_GATHER:
			String msgStr;
			if (leaderAgent == id())
			...
			setState(AgentStates.S_RA_ASSIGN);
			break;
		case R_RA_ASSIGN:		
			if (leaderAgent != id())
			{
...
/*******************************************************************/
	...
	protected void logInf(String msg) {
		if (dbgInf) {
			...
			try {
			    ...
			    out.println("[NoHelpAssignAgent " + id() + "]: " + msg);
			    out.close();
			} catch (IOException e) {
	...
	private void logErr(String msg) {
		if (dbgErr)
			System.err.println("[xx][NoHelpAssignAgent " + id() + 
							   "]: " + msg);
	}
	 ...
	 */
	private void setState(AgentStates newState) {
